'use strict';
const userKey = '36375407-b186fdf61b14328b3f73ebdf3';
...
const renderImages = data => {
  ...
  const element = data
    .map(elem => {
      ...
      return `
      <div class="photo-card__items">
      <figure>
       <a class="photo-card__item" href=${elem.largeImageURL}><img class="photo-card__image" src="${elem.webformatURL}" alt="" loading="lazy" /></a>
...
const handleScroll = autoScrollFlag => {
  if (autoScrollFlag) {
    ...
    if (endOfPage && galleryItemsRendered < galleryItemsLimit) loadmore(); // sprawdzam czy znalazłam się na koncu strony oraz czy wyrenderowane zostały wszystkie obrazy
  }
};
